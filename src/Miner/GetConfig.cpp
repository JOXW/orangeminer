...
Pool getPool()
{
    ...
    while (true)
    {
        std::cout << InformationMsg("Enter the pool address to mine to.") << std::endl
                  << InformationMsg("This should look something like pool.securuscoin.org:5555 ");
        ...
        {
            std::cout << WarningMsg("Invalid pool address! Should be in the form host:port, for example, pool.securuscoin.org:5555!")
                      << std::endl;
...
MinerConfig getMinerConfig(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (version)
        {
            std::cout << "orangeminer " << Constants::VERSION << std::endl;
            exit(0);
        }
